...
`--since-compact` exports only what came after the most recent context
compaction, which is how to pick up a transcript where the last export
stopped instead of re-rendering the whole session.
"""
import json
import re
import sys
...
def compact_boundary(path):
    """Line number of the most recent compaction, or 0 if there is none."""
    last = 0
    ...
    with open(path, errors="ignore") as fh:
        for lineno, line in enumerate(fh):
            ...
            if role == "user" and not body.strip():
                continue
            if role == "user":
                # slash-command plumbing is not conversation: strip the
                # wrappers, and drop the turn if nothing human is left
                body = re.sub(r"<system-reminder>.*?</system-reminder>", "",
                              body, flags=re.S)
                body = re.sub(r"</?(command-name|command-message|command-args|"
                              r"local-command-stdout|local-command-caveat)>",
                              "\n", body)
                body = re.sub(r"^\s*/(compact|clear|cost|help)\b.*$", "",
                              body, flags=re.M)
                body = re.sub(r"^\s*Compacted\s*$", "", body, flags=re.M)
                body = re.sub(r"Caveat: The messages below were generated by "
                              r"the user while running local commands\..*?"
                              r"explicitly asks you to\.", "", body, flags=re.S)
                body = re.sub(r"\n{3,}", "\n\n", body).strip()
                # what survives a stripped slash command is its own name
                if body.lower() in ("compact", "clear", "cost", "help"):
                    continue
                if not body:
                    continue
...
def main():
    ...
    since = "--since-compact" in sys.argv[3:]
    start = compact_boundary(src) if since else 0
    if since and not start:
        print("no compaction found - exporting the whole session")
    turns = merge(load(src, start))
    page = A4
    margin = 16 * mm
    ...
    story = [header_band(
        width, "VENDICATOR",
        ("Build conversation &middot; part two" if since
         else "Build conversation transcript")
        + f" &middot; {len(turns)} exchanges "
